...
async def test_diff_new_table(db):
    collection = Collection("items", fields=[TextField("title")])
    comparator = SchemaComparator(db, [collection])
...
async def test_diff_no_changes(db):
    collection = Collection("items", fields=[TextField("title")])
    await db.execute(
...
async def test_diff_new_column(db):
...
async def test_diff_drop_column(db):
    collection = Collection("items", fields=[TextField("title")])
...
async def test_diff_multiple_collections(db):
    ...
    comparator = SchemaComparator(db, [col1, col2])
    ops = await comparator.diff()
    assert len(ops) == 2
...
async def test_diff_builtin_columns_excluded(db):
    collection = Collection("items", fields=[TextField("title")])
