...
logger = logging.getLogger("zork.collections.store")
...
class CollectionStore:
    ...
    async def create(
        ...
        ctx = ctx or ZorkContext(collection=collection.name, operation="create")
        ...
        try:
            ...
            raise ZorkError(400, str(exc)) from exc
    ...
    async def get(
        self, collection: Collection, id: str, ctx: ZorkContext | None = None
    ) -> dict | None:
        ctx = ctx or ZorkContext(collection=collection.name, operation="read")
    ...
    async def list(
        ...
        *,
        ...
        limit: int = 20,
        offset: int = 0,
        ctx: ZorkContext | None = None,
    ) -> tuple[list[dict], int]:
        ctx = ctx or ZorkContext(collection=collection.name, operation="list")
    ...
    async def update(
        ...
        ctx = ctx or ZorkContext(collection=collection.name, operation="update")
    ...
    async def delete(
        ...
        ctx = ctx or ZorkContext(collection=collection.name, operation="delete")
        existing = await self._raw_get(collection, id)
        if existing is None:
            return False
        try:
