"""File upload / download / delete route handlers for Zork collections.
...
if TYPE_CHECKING:
    from zork.collections.schema import Collection, FileField
    from zork.collections.store import CollectionStore

    from .backends import FileStorageBackend

logger = logging.getLogger("zork.storage")
...
def make_upload_handler(
    ...
    async def upload_file(request: Request) -> JSONResponse:
        ...
        if "multipart/form-data" not in content_type:
            raise ZorkError(415, "File upload requires multipart/form-data")

        # 3. Parse form
        try:
            form = await request.form()
        except Exception as exc:
            raise ZorkError(400, f"Failed to parse multipart form: {exc}") from exc

        upload = form.get("file")
        if upload is None:
            raise ZorkError(400, "Missing 'file' field in multipart form")
        ...
        if updated_record is None:
            raise ZorkError(500, "Failed to update record metadata")
...
def make_download_handler(
    ...
    async def download_file(request: Request) -> Response:
        ...
        if not meta:
            raise ZorkError(404, "No file uploaded for this field")

        if field.multiple:
            index_param = request.query_params.get("index")
            if index_param is None:
                raise ZorkError(400, "?index=N is required for multi-file fields")
            try:
                index = int(index_param)
            except ValueError:
                raise ZorkError(400, "?index must be an integer")
            if not isinstance(meta, list) or index < 0 or index >= len(meta):
                raise ZorkError(404, f"No file at index {index}")
        ...
        if not key:
            raise ZorkError(500, "File metadata is corrupt (missing key)")
        ...
        # 4. Proxy the bytes through Zork
        try:
            data, content_type = await backend.get(key)
        except FileNotFoundError:
            raise ZorkError(404, "File not found in storage backend")
...
def make_delete_handler(
    ...
    async def delete_file(request: Request) -> JSONResponse:
        ...
        meta = record.get(field_name)
        if not meta:
            raise ZorkError(404, "No file to delete for this field")
