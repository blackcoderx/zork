...
def _check_owner(request: Request, record: dict) -> None:
    ...
    if record.get("created_by") != user.get("id"):
        raise CinderError(403, "You do not have permission to access this record")
        ...
        if expand_fields:
            for item in items:
                ...
                for field_name in expand_fields:
                    await _expand_field(item, field_name, collection, store, all_collections)
        return JSONResponse({
    ...
    async def get_record(request: Request) -> JSONResponse:
        ...
        if expand_param:
            ...
            for field_name in expand_param.split(","):
                await _expand_field(record, field_name, collection, store, all_collections)
...
async def _expand_field(
    record: dict, field_name: str, collection: Collection,
    store: CollectionStore,
    all_collections: dict[str, tuple[Collection, dict[str, str]]],
) -> None:
